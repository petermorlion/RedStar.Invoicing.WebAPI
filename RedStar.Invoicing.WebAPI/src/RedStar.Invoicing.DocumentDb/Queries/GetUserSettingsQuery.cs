    ...
    public class GetUserSettingsQuery : IGetUserSettingsQuery
    {
        ...
        public async Task<Optional<UserSettings>> Execute(string userId)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var authHeader = masterKeyAuthorizationSignatureGenerator.Generate("GET", resourceLink, "docs", authorizationKey, "master", "1.0", utcNow);
                httpClient.DefaultRequestHeaders.Add("authorization", authHeader);

                var response = await httpClient.GetAsync(new Uri(baseUrl, resourceLink));
